...
/**
 * Typed control-plane facade for precondition-check management.
 */
@Service
public class PreconditionCheckManagementService {
    ...
    private final PreconditionCheckService preconditionCheckService;

    private final ObjectMapper objectMapper;

    public PreconditionCheckManagementService(
            PlatformSpaceService platformSpaceService,
            ConnectorService connectorService,
            PreconditionCheckService preconditionCheckService,
            ObjectMapper objectMapper) {
        this.platformSpaceService = platformSpaceService;
        this.connectorService = connectorService;
        this.preconditionCheckService = preconditionCheckService;
    ...
    public Optional<ResolvedPreconditionCheckManagementView> upsertCheck(
            ...
            String connectorCode,
            String checkCode,
            PreconditionCheckUpsertCommand command) {
        if (!StringUtils.hasText(checkCode) || command == null) {
        ...
        Optional<PreconditionCheck> existing = preconditionCheckService.findLatestEnabledByCode(
                connectorResolution.space().getId(), checkCode.trim());
        PreconditionCheck target = existing.orElseGet(PreconditionCheck::new);
        LocalDateTime now = LocalDateTime.now();
        if (target.getId() == null) {
            target.setSpaceId(connectorResolution.space().getId());
            target.setConnectorId(connectorResolution.connector().getId());
            target.setCheckCode(checkCode.trim());
        ...
        target.setCheckExpressionJson(serializeJson(command.checkExpression()));
        ...
        boolean persisted = target.getId() == null
                ? preconditionCheckService.save(target)
                : preconditionCheckService.updateById(target);
        if (!persisted) {
            return Optional.empty();
        }
    ...
    private ResolvedPreconditionCheckManagementView toResolved(
            ConnectorResolution resolution,
            PreconditionCheck check) {
        return new ResolvedPreconditionCheckManagementView(
                check.getId(),
                resolution.space().getSpaceCode(),
                resolution.environment(),
                resolution.connector().getConnectorCode(),
                check.getCheckCode(),
                parseMap(check.getOperationBindingJson()),
                parseList(check.getAllowedAuthProfilesJson()),
                parseList(check.getBindingStrategiesJson()),
                parseMap(check.getInputSchemaJson()),
                parseMap(check.getCheckExpressionJson()),
                parseMap(check.getFailurePolicyJson()),
                check.getStatus());
    }
