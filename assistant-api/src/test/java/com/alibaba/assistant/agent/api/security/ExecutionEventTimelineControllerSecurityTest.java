...
import com.alibaba.assistant.agent.api.controller.ExecutionEventTimelineController;
...
@ContextConfiguration(classes = ExecutionEventTimelineControllerSecurityTest.TestConfig.class)
@WebAppConfiguration
@ActiveProfiles("migration")
class ExecutionEventTimelineControllerSecurityTest {

    private static final String EVENTS_PATH = "/api/controlplane/spaces/finance-space/execution-runs/RUN-1/events";
    ...
    @BeforeEach
    void setUp() {
        Mockito.reset(executionEventTimelineService, platformSpaceService, migrationAuthService, authorizationService);
    ...
    @Test
    void shouldReturnUnauthorizedWhenTimelineBearerMissing() throws Exception {
    ...
    @Test
    void shouldReturnForbiddenWhenTimelineUserLacksControlplanePermission() throws Exception {
        ...
        mockMvc.perform(get(EVENTS_PATH).header(AUTHORIZATION, "Bearer token-chat-only"))
    ...
    @Test
    void shouldAllowTimelineWhenScopedAdminHasCatalogAccess() throws Exception {
        ...
        mockMvc.perform(get(EVENTS_PATH).header(AUTHORIZATION, "Bearer token-space-admin"))
    ...
    static class TestConfig {
        ...
        ExecutionEventTimelineService executionEventTimelineService() {
            return mock(ExecutionEventTimelineService.class);
        }
        ...
        @Bean
        ExecutionEventTimelineController executionEventTimelineController(
                ExecutionEventTimelineService executionEventTimelineService,
                PlatformSpaceService platformSpaceService,
                MigrationControlPlaneAuthorizationService authorizationService) {
            return new ExecutionEventTimelineController(
