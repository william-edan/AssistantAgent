...
@ContextConfiguration(classes = ExecutionApprovalControllerSecurityTest.TestConfig.class)
@WebAppConfiguration
...
class ExecutionApprovalControllerSecurityTest {
    ...
    @BeforeEach
    void setUp() {
        Mockito.reset(executionApprovalService, migrationAuthService, authorizationService);
    ...
    void shouldReturnUnauthorizedWhenBearerTokenMissing() throws Exception {
        ...
        verify(executionApprovalService, never()).listRequests(anyString(), anyString(), any(), any(), any());
    }
    ...
    @Test
    void shouldReturnForbiddenWhenScopedAdminLacksTargetSpace() throws Exception {
    ...
    @Test
    void shouldAllowAccessWhenScopedAdminHasTargetSpace() throws Exception {
        when(migrationAuthService.introspect("token-space-admin")).thenReturn(Optional.of(controlPlaneUser()));
        when(authorizationService.canManageSpaceExecutionApprovals(any(AuthenticatedUserContext.class), eq("finance-space"), eq("prod")))
                .thenReturn(true);
        when(executionApprovalService.listRequests("finance-space", "prod", null, null, null)).thenReturn(List.of());
    ...
    static class TestConfig {
        ...
        ExecutionApprovalService executionApprovalService() {
            return mock(ExecutionApprovalService.class);
        }
